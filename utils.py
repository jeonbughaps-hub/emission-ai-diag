...
STATION_MAPPING = {
    "대전": "성남동", "세종": "신흥동", "광주": "건국동", "예산": "내포", "서산": "동문동", "대산": "대산리",
    "당진": "당진시청", "천안": "성황동", "아산": "모종동", "논산": "취암동", "청주": "용암동",
    "여수": "여천동", "순천": "연향동", "광양": "중동", "목포": "용당동", "홍성": "내포"
}

LOC_COORDS = {
    "대전": (0.50, 0.55), "세종": (0.45, 0.50), "광주": (0.35, 0.85),
    "예산": (0.30, 0.45), "서산": (0.20, 0.40), "대산": (0.18, 0.38),
    "당진": (0.25, 0.40), "천안": (0.40, 0.45), "아산": (0.35, 0.43), "논산": (0.40, 0.60),
    "청주": (0.50, 0.45), "여수": (0.55, 0.95), "순천": (0.50, 0.90), "홍성": (0.30, 0.45)
}

def get_auto_station_and_coord(address: str):
    if not address: return "내포", (0.30, 0.45)
    for keyword, station_name in STATION_MAPPING.items():
        if keyword in address: return station_name, LOC_COORDS.get(keyword, (0.5, 0.5))
    return "내포", (0.5, 0.5)
...
def get_air_quality(station_name: str, api_key: str):
    ...
    # ★ API 키 이중 인코딩 방지를 위해 URL에 직접 결합
...
def generate_rich_advice(air_data: dict, target_station: str) -> str:
    ...
    pm10 = _f(air_data.get("pm10Value", 58)) if air_data else 58.0
    ...
    if o3 >= 0.09:
        ...
        p1 = f"1. 대기질 현황 및 잠재적 리스크 : 현재 대기질(오존 {o3:.3f}ppm)은 '보통' 수준이나, 오존 농도 상승이 예측됩니다."
