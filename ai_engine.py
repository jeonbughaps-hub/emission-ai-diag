...
def analyze_log_compliance(measure_images, user_industry: str, vector_db):
    ...
    # ★ 핵심: 프롬프트만 대폭 강화하여 종합의견을 전문적으로 생성하게 지시
    prompt = f"""당신은 환경부 비산배출시설 기술진단 전문 엔진입니다. (시점: {current_time})
대상 업종: {user_industry} | 적용 배출기준: {limit_text}

[진단 데이터 추출 규칙]
1. 방지시설 농도와 LDAR 합계를 정확히 추출하세요. {limit_val}ppm 초과 시 "부적합" 판정.

[전문 종합 의견 작성 지침]
반드시 아래 4가지 소제목을 포함하여 환경 전문가의 관점에서 아주 풍부하게(1,000자 내외) 작성하세요.
단순히 "적합하다"는 말이 아니라, 기술적인 근거를 들어 설명하세요.
【1. 시설관리 총평】: 등급 선정 근거와 전반적인 법규 준수 수준 진단.
【2. 방지시설 효율성 분석】: THC 농도 변화 추이와 기준치({limit_text}) 대비 안정적 운영 여부 분석.
【3. LDAR 점검 이행 평가】: 점검 누락 여부, 누출률 관리 상태 및 보수 적정성 평가.
【4. 중장기 관리 권고 사항】: 정기점검 대비 중점 관리 요소 및 시설 개선 제언.
...
{{
  ...
  "overall_opinion": "여기에 소제목형 전문가 의견을 상세하게 작성하세요."
}}
"""
    try:
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                ...
                temperature=0.2, # 전문적인 분석을 위해 약간의 온도를 높임
