...
def analyze_log_compliance(pdf_list, user_industry: str, vector_db):
    ...
    my_bar = st.progress(0.1, text="손상된 PDF도 거뜬한 최신 구글 서버망으로 다이렉트 전송 중입니다...")
    
    gfiles = []

    # ★ 서버 다운 주범이었던 fitz(PyMuPDF) 완전 제거!
    # 파이썬에서 문서를 열지 않고, 이진수(Binary) 그대로 안전하게 복사만 해서 넘깁니다.
    ...
    my_bar.progress(0.5, text="🚀 AI가 원본 서류를 정밀 분석 및 요약 중입니다. (약 1분 소요)")
