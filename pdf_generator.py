...
class ProfessionalPDF(FPDF):
    ...
    def draw_cover(self, company, address, industry, permit_no, date_str):
        ...
        for i, (k, v) in enumerate(info_rows):
            ...
            self.set_font(fn, "", 9); self.set_text_color(30, 40, 60); self.cell(130, row_h - 1, " " + str(v).replace('\n', ' '), 0, 0, "L", fill=True)
        sign_y = info_y + len(info_rows) * row_h + 10; self.set_draw_color(185, 205, 228); self.set_line_width(0.3); self.rect(20, sign_y, 172, 22, "D")
    ...
    def draw_toc(self, toc_items):
        ...
        for title, page_hint in toc_items:
            ...
            self.cell(150, 7, title.strip()); self.cell(20, 7, page_hint, 0, 1, "R")
    ...
    def draw_sub_header(self, txt):
        ...
        self.set_font(fn, "B", 10); self.set_text_color(*BRAND_ACCENT); self.set_x(12); self.cell(0, 7, txt, 0, 1, "L")
    ...
    def draw_scorecard(self, scores_data: dict):
        ...
        self.set_font(fn, "B", 10); self.set_text_color(*BRAND_NAVY); self.set_xy(15, start_y + 4); self.cell(0, 6, "항목별 진단 점수 및 종합 등급")
        total_score = scores_data.get("overall_score", {}).get("score", 0)
        total_grade = scores_data.get("overall_score", {}).get("grade", "F")
        self.set_font(fn, "B", 20); self.set_text_color(*SCORE_COLORS.get(total_grade, (100,100,100))); self.set_xy(160, start_y + 8); self.cell(30, 15, total_grade, 0, 0, "C")
    ...
    def draw_text_box(self, text: str, title: str = ""):
        ...
        self.set_font(fn, "", 9.5); self.set_text_color(50, 60, 75)
        for line in text.split("\n"):
            ...
            if re.match(r"^【.*】", line):
                ...
                self.multi_cell(w, 6, line); self.set_font(fn, "", 9.5); self.set_text_color(50, 60, 75)
            else:
...
def create_gov_report_pdf(ai_data: dict, user_info: dict, air_advice: str, air_data: dict, station_name: str) -> bytes:
    ...
    pdf.draw_text_box(air_advice, title=f"측정소: {station_name}")
    ...
    pdf.draw_zebra_table(["구분", "측정일", "시설명", "결과", "기준", "판정"], prev_rows, [25, 25, 60, 25, 25, 30], highlight_last_col=True)
