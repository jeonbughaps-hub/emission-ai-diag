...
class ProfessionalPDF(FPDF):
    ...
    def draw_toc(self, toc_items):
        ...
        for title, page in toc_items:
            ...
            clean = title.strip(); dot_w = 150 - self.get_string_width(clean)
    ...
    def draw_section_header(self, txt, set_section=True):
        ...
        self.set_font(fn, "B", 13); self.set_text_color(*BRAND_NAVY); self.set_x(16)
    ...
    def draw_sub_header(self, txt):
        ...
        self.set_font(fn, "B", 10); self.set_text_color(*BRAND_ACCENT); self.set_x(12)
        self.cell(0, 7, txt, 0, 1, "L"); self.ln(1)

    def draw_zebra_table(self, headers, rows, col_widths, highlight_last_col=False):
        fn = self._fn(); self.set_fill_color(*BRAND_HEADER_BG); self.set_draw_color(175, 195, 220)
        self.set_line_width(0.2); self.set_font(fn, "B", 9); self.set_text_color(*BRAND_NAVY)
    ...
    def draw_scorecard(self, scores_data: dict):
        ...
        self.set_fill_color(250, 251, 254); self.set_draw_color(210, 220, 238); self.set_line_width(0.35)
        self.rect(10, start_y, 190, 32, "FD")
        ...
        self.set_font(fn, "B", 24); self.set_text_color(*SCORE_COLORS.get(total_grade, (100,100,100)))
        self.set_xy(160, start_y + 8); self.cell(30, 15, total_grade, 0, 0, "C")
...
def create_gov_report_pdf(ai_data: dict, user_info: dict, air_advice: str, air_data: dict, station_name: str) -> bytes:
    ...
    toc_items = [("가. 사업장 및 진단 개요", "1"), ("나. 준수율 종합 스코어카드", "1"), ("다. 지역 환경 분석", "2"), ("라. 시설별 정밀 진단 내역 (전수조사)", "3"), ("바. AI 정밀 진단 종합 의견", "4")]
