...
class ProfessionalPDF(FPDF):
    ...
    def draw_cover(self, company, address, industry, permit_no, date_str):
        ...
        self.set_y(8); self.set_font(fn, "B", 10); self.set_text_color(220, 232, 248); self.cell(0, 8, "한국환경공단 비산배출 자가진단 시스템 (시범사업)", 0, 1, "C")
    ...
    def draw_toc(self, toc_items):
        ...
        self.set_y(20); self.set_font(fn, "B", 14); self.set_text_color(*BRAND_NAVY); self.cell(0, 10, "목    차", 0, 1, "C"); self.ln(10)
        for title, page in toc_items:
            self.set_x(30); self.set_font(fn, "B" if not title.startswith("  ") else "", 10); self.cell(140, 8, title); self.cell(10, 8, page, 0, 1, "R")
    ...
    def draw_scorecard(self, scores_data: dict):
        ...
        self.set_xy(15, start_y + 5); self.set_font(fn, "B", 10); self.set_text_color(*BRAND_NAVY); self.cell(0, 6, "항목별 진단 결과 요약")
        grade = scores_data.get("overall_score", {}).get("grade", "F")
        self.set_font(fn, "B", 24); self.set_text_color(*SCORE_COLORS.get(grade, (0,0,0))); self.set_xy(160, start_y + 5); self.cell(30, 20, grade, 0, 0, "C")
