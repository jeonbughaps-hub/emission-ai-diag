...
def create_gov_report_pdf(ai_data: dict, user_info: dict, air_advice: str, air_data: dict, station_name: str) -> bytes:
    ...
    # 28번 파일의 완벽한 9개 목차 복구
    toc_items = [
        ("가. 사업장 및 진단 개요", "1"), ("  - 1) 기본 정보 요약표", "1"),
        ("나. 준수율 종합 스코어카드", "1"), ("  - 1) 항목별 점수 및 등급", "1"),
        ("다. 공공데이터 기반 지역 환경 분석", "2"), ("  - 1) 주요 대기질 현황 및 지침", "2"),
        ("라. 시설별 정밀 진단 내역 (전수조사)", "3"), ("  - 1) 방지시설 배출농도 추이 (THC)", "3"), ("  - 2) 비산누출시설(LDAR) 점검 실적", "4"),
        ("마. 위험도 매트릭스 및 행정처분 가능성 평가", "4"),
        ("바. AI 정밀 진단 종합 의견 및 중장기 로드맵", "5"),
        ("사. 관련 규제 및 행정처분 참고사항", "6"),
        ("아. 비산배출시설 변경신고 및 추진체계", "7"),
        ("자. 자가 체크리스트 (정기 점검표)", "8")
    ]
    ...
    # 가. 개요
    pdf.add_new_section_page("가. 사업장 및 진단 개요"); pdf.draw_sub_header("1) 기본 정보 요약표")
    ...
    # 나. 스코어카드 (비주얼 블록 복구)
    pdf.ln(5); pdf.set_font(pdf._fn(), "B", 13); pdf.set_text_color(*BRAND_NAVY); pdf.cell(0, 11, "나. 준수율 종합 스코어카드", 0, 1, "L"); pdf.line(10, pdf.get_y(), 200, pdf.get_y()); pdf.ln(2)
    ...
    law2 = [["신고/변경신고 미이행", "경고", "경고", "조업정지 10일", "조업정지 20일"], ["시설관리기준 미준수", "경고", "조업정지 10일", "조업정지 20일", "조업정지 20일"], ["정기점검 미수검", "경고", "경고", "조업정지 10일", "조업정지 20
