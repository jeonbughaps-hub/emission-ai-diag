...
class ProfessionalPDF(FPDF):
    ...
    def header(self):
        ...
        fn = self._fn()
        self.set_font(fn, "B", 8); self.set_text_color(*BRAND_NAVY); self.set_xy(10, 7)
        self.cell(95, 5, "비산배출시설 정밀 자가진단 보고서", 0, 0, "L")
        self.set_x(10); self.cell(190, 5, getattr(self, '_section', ''), 0, 0, "R")
    ...
    def draw_cover(self, company, address, industry, permit_no, date_str):
        ...
        self.set_y(56); self.set_font(fn, "B", 9); self.set_text_color(*BRAND_NAVY); self.cell(0, 6, "HAPs 비산배출시설 환경관리 적합성 정밀 진단", 0, 1, "C")
    ...
    def draw_section_header(self, txt, set_section=True):
        ...
        self.ln(8); self.check_page_break(25) 
    ...
    def draw_zebra_table(self, headers, rows, col_widths):
        ...
        for row in rows:
            ...
            for i, val in enumerate(row): self.cell(col_widths[i], row_h, str(val).replace('\n', ' '), border="B", align="C", fill=True)
            self.ln(); alt = not alt
    ...
    def draw_visual_scorecard(self, scores_data: dict):
        ...
        blocks = [("관리자 선임", scores_data.get("manager_score", {}).get("score", 100), scores_data.get("manager_score", {}).get("grade", "A")), ("방지시설 기준", scores_data.get("prevention_score", {}).get("score", 95), scores_data.get("prevention_score", {}).get("grade", "A")), ("LDAR 점검", scores_data.get("ldar_score", {}).get("score", 100), scores_data.get("ldar_score", {}).get("grade", "A")), ("기록 충실성", scores_data.get("record_score", {}).get("score", 90), scores_data.get("record_score", {}).get("grade", "B"))]
        ...
        for title, score, grade in blocks:
            self.set_fill_color(255, 255, 255); self.set_draw_color(220, 220, 220); self.rect(x, start_y, block_w, 25, "FD")
            self.set_fill_color(160, 230, 180); self.rect(x + block_w - 10, start_y + 2, 8, 8, "F")
            self.set_font(fn, "B", 8); self.set_text_color(0, 100, 0); self.set_xy(x + block_w - 10, start_y + 3); self.cell(8, 6, str(grade), 0, 0, "C")
            self.set_font(fn, "", 8); self.set_text_color(100, 100, 100); self.set_xy(x + 2, start_y + 3); self.cell(block_w - 12, 6, title, 0, 0, "L")
            self.set_font(fn, "B", 18); self.set_text_color(*BRAND_NAVY); self.set_xy(x, start_y + 12); self.cell(block_w, 10, str(score), 0, 0, "C"); x += block_w + gap
        self.set_fill_color(150, 230, 180); self.rect(x, start_y, 190 - (x - 10), 25, "F")
        self.set_font(fn, "B", 9); self.set_text_color(*BRAND_NAVY); self.set_xy(x, start_y + 2); self.cell(190 - (x - 10), 6, "종합등급", 0, 0, "C")
        self.set_font(fn, "B", 24); self.set_text_color(0, 100, 0); self.set_xy(x, start_y + 9); self.cell(190 - (x - 10), 10, str(overall_grade), 0, 0, "C")
        self.set_font(fn, "", 8); self.set_text_color(*BRAND_NAVY); self.set_xy(x, start_y + 19); self.cell(190 - (x - 10), 5, f"총점 {overall_score}점", 0, 0, "C"); self.set_y(start_y + 30)

    # ★ 업그레이드된 동적 컬러 인포그래픽 (게이지 바 추가) ★
    def draw_air_quality_infographic(self, station_name: str, air_data: dict):
        fn = self._fn(); self.check_page_break(50); start_y = self.get_y()
        ...
        self.set_fill_color(245, 248, 252); self.rect(10, start_y, 190, 42, "F")
        self.set_xy(15, start_y + 4); self.set_font(fn, "B", 10); self.set_text_color(*BRAND_NAVY)
        self.cell(0, 6, f"실시간 대기질 지수 대시보드 (관할 측정소: {station_name})")

        # O3 카드
        self.set_fill_color(255, 255, 255); self.set_draw_color(210, 220, 230); self.rect(15, start_y + 12, 80, 26, "FD")
        self.set_xy(20, start_y + 15); self.set_font(fn, "B", 10); self.set_text_color(70, 80, 95); self.cell(30, 6, "오존 (O3)", 0, 0, "L")
        self.set_xy(20, start_y + 22); self.set_font(fn, "B", 14); self.set_text_color(40, 50, 60); self.cell(40, 8, f"{o3_str} ppm", 0, 0, "L")
        self.set_fill_color(*o3_color); self.rect(72, start_y + 16, 18, 6, "F")
        self.set_xy(72, start_y + 16.5); self.set_font(fn, "B", 9); self.set_text_color(255, 255, 255); self.cell(18, 5, o3_stat, 0, 0, "C")
        bar_w = 55; bar_x = 20; bar_y = start_y + 33
        ...
        self.set_font(fn, "", 7); self.set_text_color(120, 120, 120); self.set_xy(bar_x + bar_w + 3, bar_y - 1.5); self.cell(10, 5, "기준:0.09", 0, 0, "L")

        # PM10 카드
        self.set_fill_color(255, 255, 255); self.set_draw_color(210, 220, 230); self.rect(105, start_y + 12, 80, 26, "FD")
        self.set_xy(110, start_y + 15); self.set_font(fn, "B", 10); self.set_text_color(70, 80, 95); self.cell(40, 6, "미세먼지 (PM10)", 0, 0, "L")
        self.set_xy(110, start_y + 22); self.set_font(fn, "B", 14); self.set_text_color(40, 50, 60); self.cell(40, 8, f"{pm10_str} ㎍/m³", 0, 0, "L")
        self.set_fill_color(*pm10_color); self.rect(162, start_y + 16, 18, 6, "F")
        self.set_xy(162, start_y + 16.5); self.set_font(fn, "B", 9); self.set_text_color(255, 255, 255); self.cell(18, 5, pm10_stat, 0, 0, "C")
        bar_w2 = 55; bar_x2 = 110; bar_y2 = start_y + 33
        ...
        self.set_font(fn, "", 7); self.set_text_color(120, 120, 120); self.set_xy(bar_x2 + bar_w2 + 3, bar_y2 - 1.5); self.cell(10, 5, "기준:80", 0, 0, "L")
        self.set_y(start_y + 45)

    def draw_text_box(self, text: str, title: str = ""):
        fn = self._fn()
        if title: 
            self.check_page_break(15); self.set_font(fn, "B", 10); self.set_text_color(*BRAND_NAVY); self.set_x(10); self.cell(0, 7, title, 0, 1, "L")
        for line in text.split("\n"):
            line = line.strip()
            if not line: self.ln(2); continue
            if re.match(r"^【.*】", line):
                self.check_page_break(15); self.ln(2); self.set_font(fn, "B", 10); self.set_text_color(*BRAND_NAVY); self.set_x(10); self.multi_cell(0, 6, line)
            else:
                self.set_font(fn, "", 9.5); self.set_text_color(50, 50, 50); self.set_x(10); self.multi_cell(0, 6, "  " + line)
...
def create_gov_report_pdf(ai_data: dict, user_info: dict, air_advice: str, air_data: dict, station_name: str) -> bytes:
    ...
    toc_items = [("가. 사업장 및 진단 개요", "1"), ("나. 준수율 종합 스코어카드", "1"), ("다. 공공데이터 기반 지역 환경 분석", "2"), ("라. 시설별 정밀 진단 내역 (전수조사)", "2"), ("마. 위험도 매트릭스 및 행정처분 가능성 평가", "3"), ("바. AI 정밀 진단 종합 의견 및 중장기 로드맵", "3"), ("사. 관련 규제 및 행정처분 참고사항", "4"), ("아. 비산배출시설 변경신고 및 추진체계", "4"), ("자. 자가 체크리스트 (정기 점검표)", "5")]
    ...
    pdf.draw_section_header("나. 준수율 종합 스코어카드"); pdf.draw_sub_header("1) 항목별 준수율 점수 및 등급")
    ...
    pdf.draw_section_header("마. 위험도 매트릭스 및 행정처분 가능성 평가"); pdf.draw_sub_header("1) 항목별 위험도 평가")
    ...
    pdf.draw_section_header("바. AI 정밀 진단 종합 의견 및 중장기 로드맵"); pdf.draw_sub_header("1) 중장기 개선 로드맵")
    ...
    pdf.draw_sub_header("2) AI 정밀 진단 종합 의견"); pdf.draw_text_box(data.get("overall_opinion", "-"))
    
    pdf.draw_section_header("사. 관련 규제 및 행정처분 참고사항"); pdf.draw_sub_header("1) 벌칙 및 과태료 규정")
    ...
    pdf.draw_section_header("아. 비산배출시설 변경신고 및 추진체계"); pdf.draw_sub_header("1) 비산배출시설 의무 변경신고 사유")
    pdf.set_font(pdf._fn(), "", 10); pdf.set_text_color(50, 50, 50); pdf.multi_cell(0, 7, "※ 비산배출시설을 신고한 사업자는 다음 사유 발생 시 의무적으로 변경신고를 해야 합니다.\n1. 사업장 명칭 또는 대표자를 변경하는 경우\n2. 비산배출시설 관리계획을 변경하는 경우\n3. 비산배출시설을 임대, 증설, 교체 또는 일부 폐쇄하는 경우\n4. 신고서의 오기, 누락 등 변경사유가 분명한 경우\n5. 비산배출시설을 완전히 폐쇄하는 경우")
    
    pdf.draw_section_header("자. 자가 체크리스트 (정기 점검표)"); pdf.draw_sub_header("■ 비산배출시설 일상 점검 체크리스트")
